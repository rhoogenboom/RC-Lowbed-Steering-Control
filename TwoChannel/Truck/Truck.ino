...
volatile OSLLightPacket dollyPacket;
...
void loop() {
  ...
  if (Startup) {
      timer.setInterval(TRANSMISSION_INTERVAL, transmitControllerInfo);
      timer.setInterval(RECEIVING_INTERVAL, receivePlateInfo);
      Startup = false;           
  }
  timer.run();
